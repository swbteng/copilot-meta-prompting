...
> `@refine` 현재 요청만 정제(그 외 무수정) / `refine()`는 사용자 지시 전까지 고정문자열("안녕,클로드") /
> JSONL 스키마 동결 / 의존성 0(순수 JS). 테스트 파일은 `_selftest_*`로 짓고 그것만 삭제하며,
> **사용자 데이터(`observe*.log`)는 절대 삭제하지 않는다.** 자세한 내용은 `AGENTS.md` 참고.
