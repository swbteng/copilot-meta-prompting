...
## 프로젝트 개요
### 배경
LLM 코딩 어시스턴트(Copilot)의 답 품질은 프롬프트의 질에 크게 좌우되지만 사용자는 보통 짧고 모호하게 묻는다(예: "정렬 코드 짜줘"). 이 프로젝트는 사용자와 타겟 LLM 사이에 정제 계층을 끼워 넣어, 모호한 입력을 구조적이고 구체적인 고품질 프롬프트로 자동 재작성 하는 것을 목표한다.
