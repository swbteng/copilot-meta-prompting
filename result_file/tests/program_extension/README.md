...
`src/program_extension`(VS Code 확장 `swbc-prompt-refiner`)의 요구사항(명세서 `R-15`~`R-24`)에 대한 자동 테스트.
...
- 이 프로젝트가 실제 검증한 Node 20.20.2는 CLI에 넘긴 glob 문자열을 스스로 확장해주지 않으므로, 위 명령의
  glob(`tests/program_extension/src/*.test.js`)은 **따옴표로 감싸지 않는다**(감싸면 리터럴 취급돼
  "Could not find" 에러). 셸(bash/git bash)이 먼저 확장해 실제 파일 목록을 넘긴다.
...
- 각 테스트 이름에 `[R-NN]`(명세서 ID)을 박아 요구사항↔테스트↔결과를 한 줄로 추적할 수 있게 했다.

## 요구사항 ↔ 테스트 매핑

| 요구사항 | 검증 방식 | 테스트 파일 |
|---|---|---|
| R-15 @refine 가로채기 | ✅ 자동 | `chat.test.js` |
| R-16 정제본 렌더링 + 5선택지 | ✅ 자동 | `chat.test.js` |
| R-17 Allow | ✅ 자동 | `chat.test.js` |
| R-18 Modify | ✅ 자동 | `chat.test.js` |
| R-21 Try again | ✅ 자동 | `chat.test.js` |
| R-19 Use original | ✅ 자동 | `chat.test.js` |
| R-20 Cancel | ✅ 자동 | `chat.test.js` |
| R-22 수정 내역 로컬 저장(JSONL) | ✅ 자동 | `logger.test.js`, `config.test.js` |
| R-23 히스토리 조회(시간순·최근 N) | ✅ 자동 (화면 '최신순 렌더'는 webview DOM이라 시연 검증) | `logReader.test.js` |
| R-24 API 통신 실패 예외 + Fallback UI | ✅ 자동 | `refiner.test.js`(200/500/text500/연결실패/빈입력), `chat.test.js`(에러메시지 + Use original/재시도/Cancel) |

> 자동 검증 10개, 전부 자동 테스트로 커버됨(사람/시연 검증 항목 없음).
> 현재 스냅샷: **25개 테스트 전부 통과**(`test-results/program_extension/`의 `junit.xml`·`summary.txt`).

### R-24(통신 실패 예외) 자동 검증 상세
