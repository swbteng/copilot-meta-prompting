...
// 대응 요구사항: R-EX-01(가로채기) / R-EX-02(렌더링+5선택지) / R-EX-03(Allow) / R-EX-04(Modify)
//               R-EX-05(Try again) / R-EX-06(Use original) / R-EX-07(Cancel) / R-EX-11(예외 fallback)
//
...
// detailedFn을 주면 refineDetailed로 주입해 서버 성공/실패({ok})를 흉내 낼 수 있다(R-EX-11).
...
test("[R-EX-01] @refine 참가자 등록 + 미리보기 단계에선 기본 LLM으로 자동 전송하지 않음(가로채기)", async () => {
  reset();
...
test("[R-EX-02] 정제본 미리보기 + 5가지 액션(Allow/Modify/Try again/Use original/Cancel) 렌더링", async () => {
  reset();
  const chat = loadChat(async (t) => "REFINED::" + t);
  const ctx = makeContext();
  chat.registerChat(ctx);
...
test("[R-EX-03] Allow -> 정제본을 타겟 LLM에 전송 + 수정 내역 1건 기록", async () => {
  reset();
  ...
  // 로그(원본->정제본) 1건 append 됐는가 (R-EX-09와 연동)
...
test("[R-EX-04] Modify -> 정제본을 입력창에 시드(전송 아님, 사용자 편집 대기)", async () => {
  reset();
...
test("[R-EX-05] Try again -> 동일 원본으로 @refine 재요청(미리보기 갱신)", async () => {
  reset();
  const chat = loadChat(async (t) => t);
  chat.registerChat(makeContext());
...
test("[R-EX-06] Use original -> 원본을 타겟 LLM에 전송, 로그 미기록", async () => {
  reset();
  const chat = loadChat(async (t) => t);
  const ctx = makeContext();
  chat.registerChat(ctx);
...
test("[R-EX-07] Cancel -> 원본을 '@refine '를 붙여 입력창에 복원(정제 취소, @refine 컨텍스트 유지)", async () => {
  reset();
  const chat = loadChat(async (t) => t);
  chat.registerChat(makeContext());
...
test("[R-EX-11] API 통신 실패(ok:false) 시 에러 메시지 + Use original/Cancel Fallback UI 제공", async () => {
  reset();
  ...
  // Fallback UI는 'Use original' / '재시도(try again)' / 'Cancel'을 제공해야 한다(명세 R-EX-11).
...
test("[R-EX-11] Fallback의 재시도 -> 동일 원본으로 @refine 재요청(미리보기 갱신)", async () => {
  reset();
  const chat = loadChat(async (t) => t);
  chat.registerChat(makeContext());
...
test("[R-EX-11] 정제 중 예외가 나도 throw하지 않고 Fallback UI(에러 메시지 + 원본/취소) 표출(fail-open)", async () => {
  reset();
