...
// 대응 요구사항: R-24(API 통신 실패 예외 처리) + 정상 응답 처리(R-16 파이프라인 근거)
//
...
test("[R-24보강] 정상 200 응답이면 서버 정제본을 그대로 사용", async () => {
...
test("[R-24] 서버 500 에러 시 throw 없이 원본을 반환(fail-open)", async () => {
  const srv = await startServer((req, res) => {
    res.statusCode = 500;
    res.setHeader("content-type", "application/json");
...
test("[R-24] 연결 실패(서버 다운) 시에도 throw 없이 원본 반환(fail-open)", async () => {
...
test("[R-24] text/plain 500 응답도 에러로 처리해 원본 반환(상태코드 검사)", async () => {
...
test("[R-24보강] refineDetailed: 200 성공 시 ok:true + 서버 정제본", async () => {
  const srv = await startServer((req, res) => {
    res.setHeader("content-type", "application/json");
...
test("[R-24] 빈/공백 입력은 그대로 반환(서버 호출 없이 방어)", async () => {
