...
- ⚠️ 디렉터리 인자(`tests/program_extension/`)를 주면 Node가 **모듈로 해석해 실패**한다. 반드시
  **glob을 따옴표로 감싼다**: `"tests/program_extension/src/*.test.js"`.
...
node --test --test-name-pattern="R-EX-11" "tests/program_extension/src/*.test.js"
```
...
| 증상 | 원인 / 해결 |
|---|---|
| `Cannot find module .../tests/program_extension` | 디렉터리를 인자로 줌 → glob `"tests/program_extension/src/*.test.js"`로 |
