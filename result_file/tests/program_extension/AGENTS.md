...
요구사항 **명세서 `R-15`~`R-24`**에 대한 자동 테스트다. 여기서 테스트를 추가/수정하는 **모든 AI 에이전트**가 따라야 할
...
1. **요구사항 ID를 테스트 이름에 박는다** — 모든 `test("...")` 제목은 명세서 ID `[R-NN]`(예: `[R-15]`)으로 시작한다.
   채점 Agent가 JUnit XML의 테스트명에서 요구사항을 역추적한다. **빠지면 매칭 실패 = 감점.**
2. **검증 대상은 실제 `src/program_extension/src/*.js`** — 테스트용 사본을 만들지 않는다. 테스트는 소스와
   같은 구조로 `tests/program_extension/src/`에 두고(소스 `src/program_extension/src/`의 미러), 항상 상대경로
...
테스트는 소스 `src/program_extension/src/`를 그대로 미러해 **`tests/program_extension/src/`**에 둔다(경로 일치 =
채점 신호). 공통 인프라 `_helpers/`(언더스코어 = 소스 대응 없음)만 모듈 테스트 루트에 둔다.

| 파일 | 검증 대상 / 역할 | 대응 요구사항 |
|---|---|---|
| `_helpers/vscode-stub.js` | `require("vscode")` 후킹 + 캡처형 `ChatResponseStream` 목. 모든 chat/config 테스트의 토대. | (공통 인프라) |
| `src/chat.test.js` | `src/chat.js` — 참가자 등록, 미리보기 5선택지, allow/modify/try again/use original/cancel, **R-24 Fallback UI** | R-15~R-21, R-24 |
| `src/refiner.test.js` | `src/refiner.js` — `refine`/`refineDetailed`/`callRefineApi`. 로컬 http로 200/500/text500/연결실패/빈입력 | R-24(+R-16 파이프라인 근거) |
| `src/logger.test.js` | `src/logger.js` — `appendRewrite` JSONL 1줄 append/누적/폴더생성/방어 | R-22 |
| `src/logReader.test.js` | `src/logReader.js` — `readEntries` 시간순 정렬·최근 N·fail-soft | R-23 |
| `src/config.test.js` | `src/config.js` — `resolveLogDir`/`getMaxEntries` | R-22 / R-23 보강 |
...
1. 제목을 `[R-NN] <행위> -> <기대>`로 짓는다(명세서 요구사항 ID 필수).
2. 가능하면 **실코드를 그대로** 로드해 검증한다(사본 금지). vscode 의존이면 스텁을 먼저 require.
3. 자동화 불가(시각적 UI 노출 등)면 테스트를 억지로 만들지 말고, `README.md` 매핑 표에 **🧑 사람/시연
   검증**으로 적고 명세서에도 같은 표시를 단다. 명세서에 대응 요구사항이 아예 없으면 먼저 명세서에
   ID를 추가할지 요구사항에서 뺄지부터 정한다(테스트 없이 방치하지 않는다).
...
```
- 커버리지는 `--experimental-test-coverage`(내장 V8)라 추가 의존성이 없다. `--test-coverage-exclude`는
  Node 20.20.2 등 일부 버전엔 CLI 옵션 자체가 없어(`node --help`에 안 뜸) **스크립트에서 뺐다** — 그 결과
  `tests/**` 파일도 커버리지 집계에 함께 잡힌다(대부분 100%라 실질 영향은 적음). 더 신 Node에서 옵션이
  확인되면 다시 넣는다.
- 디렉터리 인자(`tests/program_extension/`)를 주면 Node가 모듈로 해석해 실패한다 → **반드시 glob**
  (`tests/program_extension/src/*.test.js`)이 필요하다. 이 Node 버전은 CLI에 넘긴 glob 문자열을 스스로
  확장해주지 않으므로(따옴표로 감싸면 리터럴 취급돼 "Could not find" 에러), `package.json` 스크립트는
  **따옴표 없이** 둬서 셸(bash/git bash)이 먼저 확장하게 한다.
