...
| `src/chat.test.js` | `src/chat.js` — 참가자 등록, 미리보기 5선택지, allow/modify/try again/use original/cancel, **R-EX-11 Fallback UI** | R-EX-01~07, R-EX-11 |
| `src/refiner.test.js` | `src/refiner.js` — `refine`/`refineDetailed`/`callRefineApi`. 로컬 http로 200/500/text500/연결실패/빈입력 | R-EX-11(+02 파이프라인 근거) |
| `src/logger.test.js` | `src/logger.js` — `appendRewrite` JSONL 1줄 append/누적/폴더생성/방어 | R-EX-09 |
| `src/logReader.test.js` | `src/logReader.js` — `readEntries` 시간순 정렬·최근 N·fail-soft | R-EX-10 |
| `src/config.test.js` | `src/config.js` — `resolveLogDir`/`getMaxEntries` | R-EX-09/10 보강 |
